...
function speedupvid
	...
	set base (echo $filename | sed 's/\.[^.]*$//')  # get the file name without the extension
	ffmpeg -i "$argv[1]" -filter_complex "[0:v]setpts=1/$argv[2]*PTS[v];[0:a]rubberband=tempo=$argv[2][a]" -map "[v]" -map "[a]" -c:v libx264 -preset ultrafast -threads 1 "$base-speed.mkv"
end
